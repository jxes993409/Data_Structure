...
int main(int argc, char *argv[])
{
  ...
  write_output(output_name, Data.return_processor_num, Data.return_processor);
  for(int i = 0; i < Data.return_processor_num; i++)
  {
    delete [] Data.return_processor[i].pq;
    delete [] Data.return_processor[i].ts;
  }
  return 0;
}

Return_Data read_file(const char filename[])
{
  FILE *input_file = fopen(filename, "r");
  char *line = new char[15];
  size_t len = 0;
  int processor_id, processor_num;
  char task_name;
  ...
  fclose(input_file);
  Return_Data Data;
  Data.return_processor_num = processor_num;
  Data.return_processor = processor;
  return Data;
}
